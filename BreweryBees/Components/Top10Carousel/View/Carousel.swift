...
extension Carousel: UICollectionViewDataSource{
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.titleLabel.text = path.name
        cell.scoreLabel.text = "\(path.average ?? 0)"
        cell.typeLabel.text = path.brewery_type?.uppercased()
        cell.imageArea.sd_setImage(with: URL(string: self.topTenViewModel.breweriesImages[indexPath.row]), placeholderImage: UIImage(named: "Vector-1"))
        cell.imageArea.contentMode = .scaleToFill
        return cell
    }
}
